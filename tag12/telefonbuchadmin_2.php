...
while($row = mysqli_fetch_assoc($result)) {
	...
	echo '<td><a href="telefonbuchadmin_2.php?edit='.$row['id'].'">Editieren</a></td>';
    echo '<td><a href="telefonbuchadmin_2.php?delete='.$row['id'].'">Löschen</a></td>';
	echo '</tr>';
}
echo '</table><br><br>';
...
if(isset($_GET['edit'])) {
	...
	if($id > 0) {
		...
		if($row = mysqli_fetch_assoc($result)) {
			echo'<form action="telefonbuchadmin_2.php" method="post">
...
if(isset($_GET['delete'])) {
	...
	if($id > 0) {
		...
		if($row = mysqli_fetch_assoc($result)) {
			echo'<form action="telefonbuchadmin_2.php" method="post">
			<input type="hidden" name="id" value="'.$row['id'].'">
            <p>Soll der Eintrag: <strong>'.$row['vorname'].' '.$row['nachname'].' '.$row['telefon'].'</strong> wirklich gelöscht werden?</p>
			<button type="submit" name="delete" value="Ja">Ja</button>
            <button type="submit" name="nein" value="Nein">Nein</button>
			</form>';
            }
		}
	}
